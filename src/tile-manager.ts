...
class TileManager {
  ...
  private sessionScreens = new Map<string, ScreenWorkArea>();
  /** Sessions with explicitly-set viewports — reflow won't override these. */
  private viewportLocked = new Set<string>();
...
'`;
  ...
  static calculateViewportFromBounds(bounds: TileBounds): { width: number; height: number } {
    return {
  ...
  async reflowAll(sessions: Map<string, Session>, multiTile?: MultiTileContext): Promise<void> {
    ...
    const screenGroups = new Map<string, { screen: ScreenWorkArea; entries: [string, Session][] }>();
    ...
    const results = await Promise.allSettled(
      Array.from(screenGroups.values()).flatMap(({ screen, entries: groupEntries }) => {
