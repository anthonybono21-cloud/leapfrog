...
- **Agent Eyes** — Green cursor dot + trailing ring tracks agent mouse movement in real-time. Scroll direction arrow appears on right edge during scrolling. Zero Node.js overhead — listens to native DOM events dispatched by Playwright. Always-on for all sessions, no env var needed.
